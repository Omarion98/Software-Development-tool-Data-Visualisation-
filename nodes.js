...
var nodes = (function() {
    ...
    var circle_click = function(graph, member) {
            ...
            var GO = {
              ...
              for (i = 0 ; i < Originalgraph2.nodes.length; i++) 
              GO.nodes.push(Originalgraph2.nodes[i]);
          
             
                 for (i = 0 ; i < Originalgraph2.links.length; i++) 
                 GO.links.push(Originalgraph2.links[i]);
          
            
            var Filtered = member.id;
            
            if (Sink.includes(String(Object.values(Filtered)))) return;
        
            // node suchen
            for (i = 0; i < GO.nodes.length; i++) {
                
              if (GO.nodes[i].id == Filtered) {
                // links filteren
                ...
                while (Bool) {
                  ...
                  let n = 0; 
                  for (j = 0; j < GO.links.length; j++) {
                    
                    let A = GO.links[j].source
                    let B = GO.links[j].target
